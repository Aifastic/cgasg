class Obra {
  ...
  actualizarCaminantes(altura, haySonido, durSonido) {
    for ( let i=0; i<this.cantCirculosTinte; i++ ) {
      this.a[i].mover(altura, haySonido, durSonido);
    }
    
  }

  actualizarSeries(intensidad){
    
    for (let i=0; i<this.seriesCirculos.length; i++) {
      this.seriesCirculos[i].actualizar(intensidad);
    }
  }
