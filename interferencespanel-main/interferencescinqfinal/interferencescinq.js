...
let fuenteMonitor;
let modoPitch = true;
let estaActivaCuentaRegresiva = false;
...
let umbralDuracionSilbido = 70;
let cuentaRegresiva = umbralDuracionSilbido;
...
function draw() {
  ...
  difIntensidad = gestorAmp.derivada * 10;
  ...
  if (estado == "comenzar"){
    ...
    obra.dibujar(/*intensidad, altura, haySonido, durSonido, umbralDuracionSonido*/);
    if (modoPitch){
      if(haySonido){
        obra.actualizarCaminantes(altura, haySonido, durSonido);
      }
    }else{
        obra.actualizarSeries(intensidad);
    }
  }

  if (estaActivaCuentaRegresiva == true)
    cuentaRegresiva --;

  if(difIntensidad >= 10 && frec >= 1000){
    estaActivaCuentaRegresiva = true;  
  }
  
  if( cuentaRegresiva <= 0 ){
      modoPitch = !modoPitch;
      reiniciarTiempo();
      estaActivaCuentaRegresiva = !estaActivaCuentaRegresiva;
    }
  ...
  antesHabiaSonido = haySonido; //guardo el estado anterior
  print(estaActivaCuentaRegresiva);
}
...
function reiniciarTiempo(){
  cuentaRegresiva = umbralDuracionSilbido;
}
